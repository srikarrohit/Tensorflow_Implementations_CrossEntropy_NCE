...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  #print(labels)
  ws = 2*skip_window + 1
  fixed_window = collections.deque(maxlen=ws)
  for i in range(ws):
    fixed_window.append(data[data_index])
  ...
  for i in range(int(batch_size/num_skips)):
    ...
    while j<num_skips:
      left = skip_window-1
      p_left = (skip_window-(target-left))//(2*skip_window)
      n_left = p_left*num_skips
      for g in range(0,n_left+1):#generating
        batch[i * num_skips + j] = fixed_window[skip_window] #fixed
        labels[i * num_skips + j, 0] = fixed_window[left]
        j=j+1
        if j>=num_skips:
          break
      right = skip_window+1
      p_right = (skip_window-(right-target))//(2*skip_window)
      n_right = p_right*num_skips
      for g in range(0,n_right+1): 
        batch[i * num_skips + j] = fixed_window[skip_window] #fixed
        labels[i * num_skips + j, 0] = fixed_window[right]
        j=j+1
        if j>=num_skips:
          break
      right = right + 1
